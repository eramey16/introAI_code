...
def distInDirection(position, position2):
    "The Manhattan distance heuristic for a PositionSearchProblem"
    xy1 = position
    xy2 = position2
    return {'leftright': (xy1[0] - xy2[0]), 'updown': (xy1[1] - xy2[1])}
...
def foodH(pt0, pts):
    ...
    maxDistanceD = 0
    for pt in pts:
        dictN = distInDirection(pt0, pt)
        lr = dictN['leftright']
        ud = dictN['updown']
        if lr < 0:
            if abs(lr) > maxDistanceL:
                maxDistanceL = abs(lr)
        else:
            if lr > maxDistanceR:
                maxDistanceR = lr
        if ud  < 0:
            if abs(ud) > maxDistanceD:
                maxDistanceD = abs(ud)
        else:
            if ud > maxDistanceU:
                maxDistanceU = ud
    sum = maxDistanceL + maxDistanceD + maxDistanceR + maxDistanceU
    return sum
...
def foodHeuristic(state, problem):
    ...
    #h1 = farthest(position, foodGrid.asList())
    #h2 = len(foodGrid.asList())
    #return max(h1, h2)
    h1 = foodH(position, foodGrid.asList())
    
    # heuristic will be the shortest path to the farthest dot
    #f = farthest(position, foodGrid.asList())[1]
    #h2 = mazeDistance(position, f, state)
    return h1
...
def closest(pt0, pts, state):
    minD = float('infinity')
    path = None
    for pt in pts:
        d = mazeDistance(pt0, pt, state)
        if d[0]<minD:
            minD=d[0]
            path = d[1]
    return path
...
class ApproximateSearchAgent(Agent):
    ...
    def getAction(self, state):
        ...
        self.actions = closest(pos, state.getFood().asList(), state)
        return self.actions.pop(0)
